...
# Load YOLO model
# -------------------------------
model = YOLO("yolov8n.pt")  # small model for CPU, use yolov8l.pt for better accuracy
...
with open(csv_file, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Time", "ObjectID", "Label", "X", "Y", "Speed (km/h)"])

# -------------------------------
# Variables
# -------------------------------
object_positions = defaultdict(list)
PIXEL_TO_METER = 0.05  # adjust this for real world
        ...
        for box in results[0].boxes:
            ...
            if obj_id is not None:
                ...
                if len(object_positions[obj_id]) >= 2:
                    ...
                    time_elapsed = t2 - t1
                    if time_elapsed > 0:
                        speed = (dist_meters / time_elapsed) * 3.6  # m/s → km/h
